<?php
	//print_r($tenant_owner);
	/*
		m1=>$tenant_owner_access,
		m2=>$utility_access,
		m3=>$billing_access,
		m4=>$cc_letter_access
	*/

//echo $m1->tenant_owner[1]
	//echo $m2;
	//echo $tenant_owner[1];
?>
<fieldset class="module" id="tenant-owner">
	<legend>Tenant/Owner</legend>
	<ol>
		<?php
			if($m1->tenant_owner[0])
				echo "<li>".anchor('tenancy/tenant_owner_type',"Tenant/Owner Type")."</li>";

			if($m1->tenant_owner[1])
				echo "<li>".anchor('tenancy/tenant_owner_particulars',"Tenant/Owner Particulars")."</li>";
			
			if($m1->tenant_owner[2])
				echo "<li>".anchor('tenancy/tenant_owner_agreement',"Tenant/Owner Agreement")."</li>";
			
			if($m1->tenant_owner[3])
				echo "<li>".anchor('tenancy/tenant_owner_lot_details',"Tenant/Owner (Lot) Details")."</li>";
			
			if($m1->tenant_owner[4])
				echo "<li>".anchor('tenancy/end_of_tenant_ownership',"End of Tenant/Ownership")."</li>";
			
			if($m1->tenant_owner[5])
				echo "<li>".anchor('tenancy/scheduled_meetings',"Scheduled Mettings")."</li>";
			
			if($m1->tenant_owner[6])
				echo "<li>".anchor('tenancy/g_l_interface',"G/L Interface")."</li>";
			
			if($m1->tenant_owner[7])
				echo "<li>".anchor('tenancy/invoice_type',"Invoice Type")."</li>";
			
			if($m1->tenant_owner[8])
				echo "<li>".anchor('tenancy/deposit_type',"Deposit Type")."</li>";
			
			if($m1->tenant_owner[9])
				echo "<li>".anchor('tenancy/tenant_owner_status',"Tenant/Owner Status")."</li>";
			
			if($m1->tenant_owner[10])
				echo "<li>".anchor('tenancy/tax_master',"Tax Master")."</li>";
			
			if($m1->tenant_owner[11])
				echo "<li>".anchor('tenancy/blr_interest',"BLR Interest")."</li>";
		?>
	</ol>
</fieldset>

<fieldset id="utilities" class="module">
